...
static std::string make_multipart_upload_body_without_name(const std::string &boundary, const std::string &content) {
    std::string body;
    body.append("--").append(boundary).append("\r\n");
    body.append("Content-Disposition: form-data; filename=\"test.txt\"\r\n");
    body.append("Content-Type: text/plain\r\n\r\n");
    body.append(content).append("\r\n--").append(boundary).append("--\r\n");
    return body;
}
...
TEST(http_server, upload_reserved_header) {
    ...
    server->onWorkerStart = [&t, &probe_file, &upload_content](Server *server, Worker *worker) {
        t = std::thread([server, &probe_file, &upload_content]() {
            ...
            std::string body = make_multipart_upload_body(boundary, upload_content, probe_file);
...
TEST(http_server, upload_reserved_header_with_preprocessor) {
    std::thread t;
    std::string probe_file = std::string(TEST_TMP_FILE) + ".upload_marker." + std::to_string(getpid());
...
TEST(http_server, upload_reserved_header_without_name) {
    std::thread t;
    std::string upload_content(80 * 1024, 'B');
    std::string upload_tmp_dir = std::string(TEST_TMP_FILE) + ".upload_dir." + std::to_string(getpid());
    ...
    server->onWorkerStart = [&t, &upload_content, &upload_tmp_dir](Server *server, Worker *worker) {
        t = std::thread([server, &upload_content, &upload_tmp_dir]() {
            swoole_signal_block_all();

            std::string boundary = "------------------------d3f990cdce762596";
            std::string body = make_multipart_upload_body_without_name(boundary, upload_content);
            std::string request = make_multipart_upload_request(boundary, body);
            SyncClient c(SW_SOCK_TCP);
            c.connect(TEST_HOST, server->get_primary_port()->port);
            c.send(request.c_str(), request.length());
            ...
            EXPECT_EQ(rmdir(upload_tmp_dir.c_str()), 0);
